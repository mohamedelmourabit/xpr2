...
public class ColisSpecification extends GenericSearchSpecification<Colis> implements Specification<Colis> {
	...
	public Predicate toPredicate(Root<Colis> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
		...
		if (colisSearch.getMc() != null) {
			...
			Predicate villeDestination = cb.like(root.join("villeDestination").get("nom"),
					"%" + colisSearch.getMc() + "%");
			...
			Predicate produit = cb.like(root.join("ligneColis").join("produit", JoinType.LEFT).get("nom"),
					"%" + colisSearch.getMc() + "%");

			Predicate produitFromVariante = cb.like(
					root.join("ligneColis").join("variante", JoinType.LEFT).join("produit", JoinType.LEFT).get("nom"),
					"%" + colisSearch.getMc() + "%");

			Predicate varianteSku = cb.like(root.join("ligneColis").join("variante", JoinType.LEFT).get("sku"),
					"%" + colisSearch.getMc() + "%");

			Predicate prix = cb.like(root.join("ligneColis").get("prix").as(String.class),
					"%" + colisSearch.getMc() + "%");

			Predicate entiteName = cb.like(root.join("entite").get("nom"), "%" + colisSearch.getMc() + "%");

			Predicate entiteId = cb.like(root.join("entite").get("id").as(String.class),
					"%" + colisSearch.getMc() + "%");

			predicates.add(cb.or(numCommande, codeEnvoi, idIntern, typeLivraison, nomComplet, destinataire, telephone,
					villeDestination, secteur, adresse, clientName, clientId, statut, produit, produitFromVariante,
					varianteSku, prix, entiteName, entiteId, remarque));
		...
		if (colisSearch.getVilleDestination() != null) {
			if (colisSearch.getVilleDestination().getNom() != null) {
				predicates.add(
						cb.equal(root.join("villeDestination").get("nom"), colisSearch.getVilleDestination().getNom()));
			}
		}
		...
		if (colisSearch.getProduitName() != null) {
			...
			predicates.add(cb.equal(root.join("ligneColis").join("variante").join("produit").get("nom"),
					colisSearch.getProduitName()));
		...
		if (colisSearch.getVarianteName() != null) {
			...
			predicates
					.add(cb.equal(root.join("ligneColis").join("variante").get("sku"), colisSearch.getVarianteName()));
		...
		if (colisSearch.getEntite() != null) {
			...
			if (colisSearch.getEntite().getId() != null) {
				predicates
						.add(cb.equal(root.join("entite").get("id").as(String.class), colisSearch.getEntite().getId()));

			}

			if (colisSearch.getEntite().getNom() != null) {
				predicates.add(
						cb.equal(root.join("entite").get("nom").as(String.class), colisSearch.getEntite().getNom()));
		...
		if (colisSearch.getPeriode() != null) {
			if (colisSearch.getPeriode().toString().split("TO").length == 2) {
				...
				try {
					...
					if (dateBefore != null && dateAfter != null) {
						Predicate dateBeforePredicate = cb.greaterThanOrEqualTo(root.<Date>get("createdDate"),
								dateBefore);
