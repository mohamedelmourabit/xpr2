...
public class JWTAuthenticationFilter extends UsernamePasswordAuthenticationFilter {
	...
	@Override	protected void successfulAuthentication(HttpServletRequest request, HttpServletResponse response, FilterChain chain,
		...
		if (utilisateurRepository == null && clientRepository == null && accountService == null) {
			...
			String jwt = Jwts.builder().setSubject(u.getEmail())
					.setExpiration(new Date(System.currentTimeMillis() + SecurityConstants.EXPIRATION_TIME))
					.signWith(SignatureAlgorithm.HS256, encodedString).claim("profiles", profiles)
					.claim("roles", authResult.getAuthorities()).claim("cni", u.getCni())
					.claim("clientId", u.getClient().getIce()).claim("clientName", u.getClient().getNom())
					.claim("prenom", u.getPrenom()).claim("nom", u.getNom())
					.claim("typeUtilisateur", u.getTypeUtilisateur()).claim("entiteId", u.getEntite().getId())
					.claim("entiteName", u.getEntite().getNom()).compact();
