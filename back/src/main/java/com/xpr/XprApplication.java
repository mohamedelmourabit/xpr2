...
import com.xpr.entities.Villes;
...
public class XprApplication implements CommandLineRunner {
	...
	public void run(String... args) throws Exception {
		...
		// preparer les donnes de villes
		
		Villes villes = new Villes();
		try(InputStream input  = classloader.getResourceAsStream("villes.json")){
			final  BufferedReader streamReader = new  BufferedReader(new InputStreamReader(input, "UTF-8"));
			final StringBuilder builder = new StringBuilder();
			
			for(String line = streamReader.readLine(); line != null; line = streamReader.readLine()){
				builder.append(line);
			}
			villes = new ObjectMapper().readValue(builder.toString(), Villes.class);
		} catch (final Exception e) {
			e.printStackTrace();
		}
		
		villeRepository.saveAll(villes.getVilles());
		Entite XPR = new Entite();
		XPR.setNom("XPR");

		XPR = entiteRepository.save(XPR);

		Entite MEL = new Entite();
		MEL.setNom("MEL");

		MEL = entiteRepository.save(MEL);

		Entite CTM = new Entite();
		CTM.setNom("CTM");

		CTM = entiteRepository.save(CTM);

		for (int i = 0; i < 10; i++) {
			Client client = new Client();
			client.setIce("IdCE" + i);
			client.setNom("Client " + i);
			if (i % 2 == 0) {
				client.setEntite(MEL);
			} else {
				client.setEntite(CTM);
			}
		...
		StatutColis statutColisEnAttenteRamassage = statutColisRepositoy
				.save(new StatutColis(Constants.EN_ATTENTE_RAMASSAGE));
		StatutColis statutColisEnAttenteLivraison = statutColisRepositoy
				.save(new StatutColis(Constants.EN_ATTENTE_LIVRAISON));
		StatutColis statutColisNouveauColis = statutColisRepositoy.save(new StatutColis(Constants.NOUVEAU_COLIS));
		StatutColis statutColisReceptionneSurAgenceFES = statutColisRepositoy
				.save(new StatutColis(Constants.RECEPTIONNE_SUR_AGENCE));

		StatutBonRamassage statutBonRamassageEnAttenteRamassage = statutBonRamassageRepositoy
				.save(new StatutBonRamassage(Constants.EN_ATTENTE_RAMASSAGE));
		StatutBonRamassage statutBonRamassageRamasse = statutBonRamassageRepositoy
				.save(new StatutBonRamassage(Constants.RAMASSE));
		...
		profileRamasseur.getAutorisations()
				.add(this.autorisationRepository.findByAuthName("BON-RAMASSAGE-REST$LIST$OWN"));
		profileRamasseur.getAutorisations()
				.add(this.autorisationRepository.findByAuthName("BON-RAMASSAGE-REST$UPDATE$OWN"));
		 ...
		 * 
		 * LigneBonExpedition lbe = new LigneBonExpedition(); lbe.setColis(colis1);
		 * 
		 * be.getLigneBonExpeditions().add(lbe);
		 * colis1.setStatut(StatusColisEnCoursExpedition); HistoriqueBonExpedition h2 =
		 * new HistoriqueBonExpedition(); h2.setAction("creer nouveau be");
		 * h2.setBonExpedition(be); h2.setUtilisateur(utilisateurXpr);
		 * h2.setStatut(br.getStatut()); h2.setDateCreation(new Date());
		 * 
		 * be.setDepart(agenceFes); be.setDestination(agenceCasa);
		 * be.setLogistique("CTM"); be.setRefBonLogistique("reference1");
		 * be.getHistoriques().add(h);
		 * 
		 * h = new Historique(); h.setAction("en cours expedition"); h.setColis(colis1);
		 * h.setUtilisateur(utilisateurXpr); h.setStatut(br.getStatut());
		 * h.setDateCreation(new Date()); be.getHistoriques().add(h);
		 * 
		 * be = bonExpeditionRepository.save(be); System.out.println("be " +
		 * be.getNom()); historiqueRepository.save(h);
		 * 
		 * // arrive vers casa
		 * 
		 * be.setStatut(Constants.EXPEDIE); h = new Historique();
		 * h.setAction("Confirmation reception expedition vers agence CASA");
		 * h.setBonExpedition(be); h.setUtilisateur(utilisateurXpr);
		 * h.setStatut(br.getStatut()); h.setDateCreation(new Date());
		 * be.getHistoriques().add(h); historiqueRepository.save(h);
		 * 
		 * colis1.setStatut(StatusColisExpedie);
		 * 
		 * h = new Historique(); h.setAction("Arrivé sur Casablanca");
		 * h.setColis(colis1); h.setUtilisateur(utilisateurXpr);
		 * h.setStatut(br.getStatut()); h.setDateCreation(new Date());
		 * colis1.getHistoriques().add(h);
		 * 
		 * be = bonExpeditionRepository.save(be); historiqueRepository.save(h);
		 * 
		 * // affectation au livreur receptioniste
		 * 
		 * colis1.setLivreur(livreur2);
		 * colis1.setStatut(StatusColisEnAttenteLivraisonClient); h = new Historique();
		 * h.setAction("Affectation au livreur"); h.setColis(colis1);
		 * h.setUtilisateur(utilisateurXpr); h.setStatut(br.getStatut());
		 * h.setDateCreation(new Date()); colis1.getHistoriques().add(h);
		 * historiqueRepository.save(h);
		 * 
		 * // Livraison client
		 * 
		 * colis1.setLivreur(livreur2); colis1.setStatut(statutColisLivre);
		 * colis1.setDateLivraison(new Date()); // scanné colisRepository.save(colis1);
		 * h = new Historique(); h.setAction("Livré au client"); h.setColis(colis1);
		 * h.setUtilisateur(livreur2); h.setStatut(br.getStatut());
		 * h.setDateCreation(new Date()); colis1.getHistoriques().add(h);
		 * historiqueRepository.save(h);
		 * 
		 ...
		 * 
		 * // type reglement : espece ou virement Facture facture = new Facture();
		 ...
		 * 
		 * ligneFacture.setFacture(facture); ligneFacture.setLigneColis(ligneColis);
		 * 
		 * double prix = 0.0; if (ligneColis.getQteLivre() != null) if
		 * (ligneColis.getQteLivre() > 0) {
		 * ligneFacture.setQte(ligneColis.getQteLivre()); if (ligneColis.getProduit() !=
		 * null) { prix = ligneColis.getProduit().getPrixVente(); } else { prix =
		 * ligneColis.getVariante().getPrixVente(); } ligneFacture.setPrixFacture(prix);
		 * }
		 * 
		 * }
		 * 
		 * facture.setLivreur(livreur2); facture.setCreerPar(livreur2);
		 * 
		 * facture.setStatut("Non reglé");
		 * 
		 * facture = factureRepository.save(facture);
		 * 
		 * colis1.setFacture(facture); colisRepository.save(colis1);
		 * 
		 * // puisque le client possede deja une facture FactureClient factureClient =
		 * new FactureClient();
		 * 
		 ...
		 * 
		 * for (LigneColis ligneColis : colis1.getLigneColis()) { LigneFactureClient
		 * ligneFactureClient = new LigneFactureClient(); if (ligneColis.getQteLivre()
		 * != null) if (ligneColis.getQteLivre() > 0) {
		 * 
		 * double prix = 0; ligneFactureClient.setQte(ligneColis.getQteLivre()); if
		 * (ligneColis.getProduit() != null) { prix =
		 * ligneColis.getProduit().getPrixVente();
		 * ligneFactureClient.setLibelle("Livraison " +
		 * ligneColis.getProduit().getNom());
		 * 
		 * } else { prix = ligneColis.getVariante().getPrixVente();
		 * ligneFactureClient.setLibelle("Livraison " +
		 * ligneColis.getVariante().getNom() + "_SKU_" +
		 * ligneColis.getVariante().getSku());
		 * 
		 * } ligneFactureClient.setPrixFacture(prix); }
		 * 
		 * if (ligneColis.getQteLivre() != null) if (ligneColis.getQteRetourne() > 0) {
		 * ligneFacture.setQte(1);
		 * 
		 * if (ligneColis.getProduit() != null) {
		 * ligneFactureClient.setLibelle("Annulation " +
		 * ligneColis.getProduit().getNom()); } else {
		 * ligneFactureClient.setLibelle("Annulation " +
		 * ligneColis.getVariante().getNom() + "_SKU_" +
		 * ligneColis.getVariante().getSku());
		 * 
		...
		/*
		 * Stock s = new Stock(); s.setAgence(agenceFes); s.setClient(client1);
		 * s.setVille(fes); s.setProduit(variante1); s.setQte(50); s.setQteNonLivre(50);
		 * s.setQteLivre(0); s.setQteEnCoursLivraison(0);
		 * 
		 * s = stockRepository.save(s);
		 * 
		 * // generation bon rammassage par le client
		 * 
		 * HistoriqueStock hs = new HistoriqueStock(); hs.setCreationDate(new Date());
		 * hs.setQte(s.getQte()); hs.setUtilisateur(utilisateurXpr); hs.setStock(s);
		 * hs.setAction("Creation du stock sur agence variante: " + variante1.getSku() +
		 * "agence=" + agenceFes.getNom()); historiqueStockRepository.save(hs);
		 * 
		 * // Creation stock agence avec variante2
		 * 
		 * Stock s2 = new Stock(); s2.setAgence(agenceFes); s2.setClient(client1);
		 * s2.setVille(fes); s2.setProduit(variante2); s2.setQte(500);
		 * 
		 * s2.setQteNonLivre(500); s2.setQteLivre(0);
		 * 
		 * s2 = stockRepository.save(s2);
		 * 
		 * // Creation stock livreur 20 variante1
		 * 
		 * int qteVarianteReserveStock = 20;//
		 * varianteStockRepository.getSumQteStockVariante(variante1.getSku());
		 * 
		 * int stockAvailable = variante1.getQte() - qteVarianteReserveStock;
		 * 
		 * if (stockAvailable >= 20) {
		 * 
		 * Stock stockLivreur = new Stock(); // stockLivreur.setLivreur(livreur1);
		 * stockLivreur.setProduit(variante1); stockLivreur.setClient(client1);
		 * stockLivreur.setVille(fes); stockLivreur.setQte(20);
		 * stockLivreur.setQteNonLivre(20); stockLivreur.setQteLivre(0);
		 * 
		 * stockLivreur = stockRepository.save(stockLivreur); hs = new
		 * HistoriqueStock(); hs.setCreationDate(new Date()); hs.setQte(s.getQte());
		 * hs.setUtilisateur(utilisateurXpr); hs.setStock(s); hs.setAction(
		 * "Creation du stock sur livreur variante: " + variante1.getSku() + "livreur="
		 * + livreur1.getCni()); historiqueStockRepository.save(hs);
		 * 
		 * }
		 * 
